...
public class FixedPaymentCalculatorTest {
    ...
    public FixedPaymentCalculatorTest(String client, double totalLoan, double interestPerYear, int years, double expectedResult) {
        this.client = client;
        this.totalLoan = totalLoan;
        this.numberOfPayments = years * 12;
        this.interestPerPayment = interestPerYear / 1200;
        this.expectedResult = new Money(expectedResult);
    }

    @Test
    public void testFixedPaymentPerMonthForSetOfCustomers_valid() {
        Assert.assertEquals(expectedResult, FixedPaymentCalculator.calculateFixedPayment(new Money(totalLoan), numberOfPayments, new Money(interestPerPayment)));
    }
}
